...
import { getArticle } from '../services/mainSlice';
import classes from './OneArticleCard.module.css';

export const OneArticleCard = ({addNotification}) => {
  const { article, username } = useSelector((state) => state.main);
  const dispatch = useDispatch();
  const currentSlug = window.location.href.split('/')[4];
  
  const createdDate = article?.createdAt ? format(new Date(article.createdAt), 'MMMM d, yyyy') : '' ;

  let historyObj = useNavigate();
  const handleEditArticle = () => {
    if(username === article.author?.username) {
      historyObj(`/articles/${currentSlug}/edit`)
    } else return;
  }
    ...
    <div className={classes.ArticleCard}>
      <div className={classes.left}>
        <div className={classes.twoValue}>
          <div className={classes.nameArticle}>{article?.title}</div>
          <div className={classes.countLikes}>
            <div>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <defs>
                  <clipPath id="clip0_9605_1">
                    <rect width="16" height="16" fill="white" />
                  </clipPath>
                </defs>
              </svg>
            </div>
            <div className={classes.favoriteCount}>{article?.favoritesCount}</div>
          </div>
        </div>
      ...
      <div className={classes.right}>
        <div className={classes.info}>
          ...
          <div className={classes.user}>{article.author?.username}</div>
        ...
        <div className={classes.buttons}>
          ...
          />
          <button className={classes.edit} onClick={() => handleEditArticle()}>Edit</button>
        </div>
      </div>
    </div>
  );
